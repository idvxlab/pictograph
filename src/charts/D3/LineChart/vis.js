...
        a = '.'+props.chartId;
    ...
    //默认卡片背景
    let cardcolor;
    let textcolor= props.textcolor == undefined ? 'rgb(0,0,0)': props.textcolor;
    ...
    if (hasSeries) {
       ...
       // linetype ="MultiSeriesLine"  
        ...
        //     .style('fill', (d, i) => colorset[i])
         ...
         //   .attr("id","pictoLayer")
            ...
            .attr("x",function(d,i){
                ...
                return margin/2-Math.abs(typex[aa]*scale1[aa])+iconindex1[aa]*1.2;
            })
            .attr("y",function(d,i){
            let aa=i;                  
            if(i>=seriesreading.length) aa=seriesreading.length-1;
                //return height-Math.abs(typex[aa]*scale1[aa])-typesizey[aa];
                return height-Math.abs(typey[aa]*scale1[aa])-typesizey[aa]+d3.max(typesizey)*(-0.1);
            })
        ...
        if(props.chartId.indexOf("sugess")<0){       
            ...
            // Y channel
            yScale = d3.scaleLinear()
                .domain([0, d3.max(averageData.map(d=>d['y']))])
                .range([chartHeight, 0])
                .nice();
           ...
           // console.log('colorPair[fieldreading[0]]', fieldreading[0])
            ...
            svg.append("g")
                ...
                //draw legend
                if(props.chartId.indexOf("sugess")<0){       
                    drawLegend([encoding.y.field],textcolor,svg,props.colorPair,width,height,props.changeElementColor,textfont)
                }
            ...
            // Y channel
            yScale = d3.scaleLinear()
                .domain([0, d3.max(averageData.map(d=>(parseInt(d['y']) + 50)))])
                .range([chartHeight, 0])
                .nice();
        ...
        for(let i=0;i<colorlen;i++){
            ...
            .append("path")
            .attr("d",function(){
                return pictorialtype[fieldreading]; 
            }) 
