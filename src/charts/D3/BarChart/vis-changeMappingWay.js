...
// import { colorPair } from '../../../selectors/canvas';
// const config = {
...
const draw = (props) => {
    ...
    console.log('props-barchart',props);
    if (!props.onCanvas) {
    ...
    console.log('bar-vis-encoding',props.spec.encoding)
    ...
    console.log("COLOR-then-then",colorPair);
...
//    categoriesreading=["add-list","creek--v1","fountain","jazz","one-to-one","water-cooler","torrent","food-and-wine"];
//    seriesreading =  ["add-list","creek--v1","fountain","jazz","one-to-one","water-cooler","torrent","food-and-wine"];
   ...
   svg.append("defs")
    ...
    .attr("id",function(d){
        if(props.chartId.indexOf("sugess")>0) return `suggess-${d}`;
            ...
            .append("g")
            .attr("id",function(d){
                if(props.chartId.indexOf("sugess")>0) return `suggesss-${d}`;
                else return `canvass-${d}`
                    })
                    .append("path")
                    .attr("d",function(d){
                        var obj = pictorialtype.find(function (obj) {
...
// Bars
            ...
            .attr("transform", function(){
                ...
                //   console.log("a平方："+area12)
                  scale1.push(Math.sqrt(area12/area11) ) 
                    return  `scale(${Math.sqrt(area12/area11)})`
    ...
    .attr("fill",(d,i)=>colorPair[categoriescolor[i]])
...
var texts=textpercent.selectAll("text")
    ...
    if(props.chartId.indexOf("sugess")<0){       
        ...
        var texts=textpercent.selectAll("text")
            ...
            rect.style('stroke-width', '0')
               ...
               //计算最大值
                ...
                let iconindex="suggesss-";
        if(props.chartId.indexOf("sugess")>0) iconindex="suggesss-";
        else iconindex="canvass-"

                // //picto添加categories
                // let scale1=[];
                // for(let i=0;i<categoriesreading.length;i++){
             
                //     console.log("draw -> categoriesreading1", categoriesreading[i])
                //     let typesizex1=svg.select(`#${iconindex}${categoriesreading[i]}`).node().getBoundingClientRect().width;
                //     let typesizey1=svg.select(`#${iconindex}${categoriesreading[i]}`).node().getBoundingClientRect().height;
         ...
         .attr("id",function(d,i){
             return "icon"+seriesreading[rectmaxindex[i]]
             })
        ...
        //  })
        .style("fill",function(d,i){
            return colorPair[categoriescolor[rectmaxindex[i]]]  //colorset[rectmaxindex[i]]
        })
            ...
            .style('fill', (d, i) => colorPair[categoriescolor[i]])
            ...
            .attr("id",function(){
               ...
               for(let a=0;a<colorlen;a++){
                   ...
                   svg.append("g")
                    ...
                    .attr("fill",colorPair[categoriescolor[a]])
               }
